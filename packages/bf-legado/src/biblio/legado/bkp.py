"""
Extrai o conteúdo de um arquivo .bkp do Biblioteca Fácil (`biblio.legado.bkp`).
...
def extrair_para_pasta(bkp_path, destino) -> dict[str, int]:
    """
    Extrai o .bkp para `destino` e devolve {nome_do_arquivo: bytes}.

    Grava também `_tabelas.pkl` com todas as tabelas juntas, que é o que os
    passos seguintes consomem quando não querem reabrir 32 arquivos.
    """
    ...
    with open(destino / "_tabelas.pkl", "wb") as f:
        pickle.dump(tabelas, f)

    return {nome: len(c) for nome, c in sorted(tabelas.items())}

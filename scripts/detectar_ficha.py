...
def main():
    parser = argparse.ArgumentParser(description="Detecta e retifica ficha CIP de foto de celular")
    parser.add_argument("imagem", help="Caminho da foto")
    parser.add_argument("--out", default=None, help="Caminho da ficha retificada (padrão: <nome>_ficha.png)")
    parser.add_argument("--debug", action="store_true", help="Salva etapas intermediárias em pasta debug/")
    ...
    original = cv2.imread(str(caminho))
    if original is None:
        print(f"Não foi possível ler a imagem: {caminho}", file=sys.stderr)
        sys.exit(1)

    # Gate de nitidez
    niv = nitidez(original)
    print(f"Nitidez (var do Laplaciano): {niv:.1f}")
    if niv < 50:
        print("  AVISO: Imagem parece borrada (limiar: 50). Resultado pode ser ruim.", file=sys.stderr)
    ...
    if contorno is None:
        print("Ficha CIP nao detectada.", file=sys.stderr)
        if args.debug:
            salvar_debug(caminho.parent / "debug", original, cinza, edged, None, None, None)
            print("  Etapas intermediarias salvas em debug/")
        sys.exit(1)

    binarizada = binarizar(warp)

    # Salvar resultado
    if args.out:
        saida = Path(args.out)
    else:
        saida = caminho.with_name(caminho.stem + "_ficha.png")

    cv2.imwrite(str(saida), binarizada)
    print(f"Ficha detectada e salva em: {saida}")
    print(f"  Dimensões: {binarizada.shape[1]}×{binarizada.shape[0]} px")

    if args.debug:
        debug_dir = caminho.parent / "debug"
        debug_dir.mkdir(exist_ok=True)
        salvar_debug(debug_dir, original, cinza, edged, contorno, warp, binarizada)
